...
import TDOP.gp.terminal.AttributeGPNode;
import TDOP.gp.terminal.DoubleERC;
import TDOP.gp.terminal.PlaceOfInterestAttribute;
import TDOP.ruleoptimisation.RuleOptimizationProblem;
import ec.Individual;
//import TDOP.gp.terminals.AttributeGPNode;
//import TDOP.gp.terminals.DoubleERC;
//import TDOP.gp.terminals.PlaceOfInterestAttribute;
...
public class GPRuleEvolutionState extends SimpleEvolutionState {
	...
	public void initBasicTerminalSet() {
		...
		for (PlaceOfInterestAttribute a : PlaceOfInterestAttribute.basicAttributes()) {
...
/*
	public void initRelativeTerminalSet() {
		terminals = new LinkedList<>();
		for (PlaceOfInterestAttribute a : PlaceOfInterestAttribute.relativeAttributes()) {
			terminals.add(new AttributeGPNode(a));
		}
	}*/
	...
	public void initTerminalSetFromCsv(File csvFile) {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
            	PlaceOfInterestAttribute a = PlaceOfInterestAttribute.get(line);
				terminals.add(new AttributeGPNode(a));
            }
	 ...
	 */
	public int indexOfAttribute(PlaceOfInterestAttribute attribute) {
		for (int i = 0; i < terminals.size(); i++) {
			PlaceOfInterestAttribute terminalAttribute = ((AttributeGPNode)terminals.get(i)).getPlaceOfInterestAttribute();
	...
	public void run(int condition)
    {
        ...
        {
			long start = TDOP.util.Timer.getCpuTime();
	...
	public int evolve() {
	    ...
	    if (generation == numGenerations-1)
	        {
	    	//return the best one so far here...?
	        return R_FAILURE;
	        }
